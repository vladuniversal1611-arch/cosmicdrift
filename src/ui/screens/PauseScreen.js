...
export class PauseScreen extends Screen {
  constructor(game) {
    ...
    this.add(new PremiumButton(bx, p.y + p.h * 0.28, bw, bh, () => this.events.emit('ui:back'),
      { label: 'RESUME', colors: UI.btn.play, radius: 24, font: '900 26px system-ui, sans-serif' }));
    this.add(new PremiumButton(bx, p.y + p.h * 0.28 + bh + 16, bw, bh, () => this.events.emit('ui:openSettings'),
      { label: 'SETTINGS', colors: UI.btn.blue, radius: 24, font: '900 26px system-ui, sans-serif' }));
    this.add(new PremiumButton(bx, p.y + p.h * 0.28 + (bh + 16) * 2, bw, bh, () => this.events.emit('ui:mainMenu'),
      { label: 'MAIN MENU', colors: UI.btn.orange, radius: 24, font: '900 26px system-ui, sans-serif' }));
  }
  ...
  render(r) {
    ...
    r.text('PAUSED', p.x + p.w / 2, ry + rh / 2, { font: '900 24px system-ui, sans-serif', color: '#fff', align: 'center', baseline: 'middle' });
