...
(function (global) {
  ...
  Engine.prototype.buildOutlineLoops = function (tile) {
    ...
    for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
      ...
      const L = x - BLEED, T = y - BLEED, R = x + tile + BLEED, B = y + tile + BLEED;
  ...
  Engine.prototype.draw = function (g) {
    ...
    loops.forEach(function (loop) {
      ...
      g.strokeStyle = 'rgba(255,210,120,0.28)'; g.lineWidth = 4; g.stroke();
      g.strokeStyle = 'rgba(255,225,160,0.80)'; g.lineWidth = 1.5; g.stroke();
    });
    ...
    loops.forEach(function (loop, li) {
      ...
      for (let k = 0; k <= TRAIL; k++) {
        ...
        // toward the biome glow colour.
        if (k === 0) {
          g.shadowColor = biome.glow; g.shadowBlur = 22;
          g.fillStyle = biome.core;
          g.beginPath(); g.arc(x, y, 3.2, 0, Math.PI * 2); g.fill();
        } else {
          g.shadowBlur = 10 * decay;
          g.shadowColor = biome.glow;
          g.fillStyle = 'rgba(255,225,160,' + (0.6 * decay).toFixed(3) + ')';
          g.beginPath(); g.arc(x, y, 2.4 * decay + 0.3, 0, Math.PI * 2); g.fill();
        }
      }
      g.shadowBlur = 0;
    });
