...
(function (global) {
  ...
  // Build closed-loop polylines that trace the outer edges of the current
  // board shape, so the spark can chase along the perimeter smoothly.
  // Every solid cell contributes a directed unit-edge on any side where the
  // neighbour is a wall or off-board; edges are then stitched by matching
  // each edge's end-point to the next edge's start-point.  Multiple loops
  // fall out naturally for holed shapes (e.g. `ring` yields outer + inner).
  Engine.prototype.buildOutlineLoops = function (tile) {
    const rows = this.rows, cols = this.cols;
    const grid = this.grid;
    const solid = function (r, c) {
      if (r < 0 || r >= rows || c < 0 || c >= cols) return false;
      const n = grid[r] && grid[r][c];
      return n && !n.wall;
    };
    // Walk edges of each solid cell CLOCKWISE so all loops wind the same way.
    // top: L→R  right: T→B  bottom: R→L  left: B→T
    // Edge endpoints snap EXACTLY to tile corners (no BLEED here) so
    // neighbouring cells' edges share endpoints and the perimeter walker
    // can stitch them into closed loops.  Visual "bleed" past the tile
    // corners is added later via stroke lineWidth + lineJoin='round'.
    const edges = [];
    for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
      if (!solid(r, c)) continue;
      const x = this.cellX(c), y = this.cellY(r);
      const L = x, T = y, R = x + tile, B = y + tile;
      if (!solid(r - 1, c)) edges.push([L, T, R, T]);
      if (!solid(r, c + 1)) edges.push([R, T, R, B]);
      if (!solid(r + 1, c)) edges.push([R, B, L, B]);
      if (!solid(r, c - 1)) edges.push([L, B, L, T]);
    }
    const key = function (x, y) { return Math.round(x) + ',' + Math.round(y); };
    const startAt = {};
    edges.forEach(function (e, i) {
      const k = key(e[0], e[1]);
      (startAt[k] || (startAt[k] = [])).push(i);
    });
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      const loop = [];
      let cur = i, safety = 0;
      while (cur !== -1 && !visited[cur] && safety++ < 10000) {
        visited[cur] = true;
        loop.push([edges[cur][0], edges[cur][1]]);
        const cands = startAt[key(edges[cur][2], edges[cur][3])] || [];
        cur = -1;
        for (let j = 0; j < cands.length; j++) if (!visited[cands[j]]) { cur = cands[j]; break; }
      }
      if (loop.length >= 3) {
        loop.push([loop[0][0], loop[0][1]]);  // close the loop
        // Precompute cumulative arc-length for fast spark-position lookup.
        loop.cumLen = [0];
        let total = 0;
        for (let m = 1; m < loop.length; m++) {
          const dx = loop[m][0] - loop[m - 1][0], dy = loop[m][1] - loop[m - 1][1];
          total += Math.sqrt(dx * dx + dy * dy);
          loop.cumLen.push(total);
        }
        loop.total = total;
        loops.push(loop);
      }
  ...
  Engine.prototype.draw = function (g) {
    ...
    g.globalAlpha = 1;
    // Board outline traced into ordered closed loops, then animated with a
    // running electric spark chasing along each loop.  Perimeter is stitched
    // from directed unit-edges (each solid cell contributes an edge on any
    // side whose neighbour is a wall or off-board), then next-endpoint
    // matching walks each loop; shapes with holes (like `ring`) naturally
    // yield two loops (outer + inner) and get two independent sparks.
    // Cache is invalidated when the shape or tile size changes.
    const cacheKey = ((this.level && this.level.shapeName) || 'full') + '|' + Math.round(tile);
    if (!this._outlineCache || this._outlineCache.key !== cacheKey) {
      this._outlineCache = { key: cacheKey, loops: this.buildOutlineLoops(tile) };
    }
    const loops = this._outlineCache.loops;
    g.save();
    g.lineCap = 'round'; g.lineJoin = 'round';
    // Base static outline: soft warm-gold glow + sharp inner line.
    // Outline sits ON the tile edge (endpoints snap to tile corners so the
    // perimeter walker can stitch loops), so lineWidth is half-outside/half-
    // inside the tile.  Wider strokes here make the outer half visible past
    // the tile art.
    loops.forEach(function (loop) {
      g.beginPath();
      g.moveTo(loop[0][0], loop[0][1]);
      for (let i = 1; i < loop.length; i++) g.lineTo(loop[i][0], loop[i][1]);
      g.strokeStyle = 'rgba(255,210,120,0.35)'; g.lineWidth = 8; g.stroke();
      g.strokeStyle = 'rgba(255,230,170,0.90)'; g.lineWidth = 2.5; g.stroke();
    });
    // Running spark(s): a bright head + a fading trail of dots, one per loop.
    const tNow = this.elapsed || 0;
    const SPEED = 260;                // px/sec, feels lively but not busy
    const TRAIL = 12;                 // trail dots behind the head
    const TRAIL_GAP = 7;              // px between trail dots
    // Pick a spark palette that matches the island biome — computed once
    // per draw, not once per loop.
    const BIOMES = [
      { core: '#ffe9a0', glow: '#ffb750' }, // dawn: warm gold
      { core: '#e6f7ff', glow: '#5fd0ff' }, // frost: icy cyan
      { core: '#f4e0ff', glow: '#b48bff' }, // storm: violet
      { core: '#e0ffd6', glow: '#5fe39a' }, // emerald: jade
      { core: '#fff4c2', glow: '#ffd24d' }  // sky: gold
    ];
    const biomeIdx = ((this.level && this.level.island) || 0) % BIOMES.length;
    const biome = BIOMES[biomeIdx];
    loops.forEach(function (loop, li) {
      const total = loop.total;
      if (!total) return;
      // Stagger loops by a fixed offset so multiple sparks don't line up.
      const head = ((tNow * SPEED) + li * total * 0.37) % total;
      for (let k = 0; k <= TRAIL; k++) {
        const dist = head - k * TRAIL_GAP;
        const pos  = ((dist % total) + total) % total;
        // Walk cumLen to find the segment containing `pos`
        let seg = 0;
        while (seg < loop.cumLen.length - 1 && loop.cumLen[seg + 1] <= pos) seg++;
        const s0 = loop.cumLen[seg], s1 = loop.cumLen[seg + 1] || (s0 + 1);
        const u = (pos - s0) / (s1 - s0 || 1);
        const x = loop[seg][0] + (loop[seg + 1][0] - loop[seg][0]) * u;
        const y = loop[seg][1] + (loop[seg + 1][1] - loop[seg][1]) * u;
        const decay = 1 - k / (TRAIL + 1);
        // Head is a bright white core with a coloured halo; trail dots fade
        // toward the biome glow colour.  Sizes bumped up so the spark is
        // clearly visible on a phone screen.
        if (k === 0) {
          g.shadowColor = biome.glow; g.shadowBlur = 28;
          g.fillStyle = biome.core;
          g.beginPath(); g.arc(x, y, 5, 0, Math.PI * 2); g.fill();
        } else {
          g.shadowBlur = 14 * decay;
          g.shadowColor = biome.glow;
          g.fillStyle = 'rgba(255,230,180,' + (0.7 * decay).toFixed(3) + ')';
          g.beginPath(); g.arc(x, y, 3.5 * decay + 0.4, 0, Math.PI * 2); g.fill();
        }
      }
      g.shadowBlur = 0;
    });
  ...
  Engine.prototype.drawTile = function (g, t, x, y, tile) {
    ...
    if (t.special !== SP.NONE) {
      ...
      if (t.special === SP.RAINBOW) {
        // shimmering rainbow wash so it reads as "clears a whole colour"
        let rain = null;
        try { rain = g.createConicGradient((this.elapsed || 0) * 0.6, cx, cy); } catch (e) { rain = null; }
        if (rain) {
          ['#ff5d6c', '#ffd24d', '#5fe39a', '#5db4ff', '#c58bff', '#ff5d6c'].forEach(function (c, i, a) { rain.addColorStop(i / (a.length - 1), c); });
          g.globalAlpha = 0.6; g.fillStyle = rain; g.fillRect(bx, by, bw, bh); g.globalAlpha = 1;
        }
      } else if (t.special === SP.BOMB) {
        // concentric warning rings that breathe
        g.globalAlpha = 0.35 + pulse * 0.4; g.strokeStyle = '#fff'; g.lineWidth = Math.max(1.5, tile * 0.045);
        for (let k = 1; k <= 2; k++) { g.beginPath(); g.arc(cx, cy, tile * 0.15 * k * (0.9 + pulse * 0.18), 0, Math.PI * 2); g.stroke(); }
        g.globalAlpha = 1;
      } else {
        // directional stripes: horizontal for LINE_H, vertical for LINE_V
        const horiz = t.special === SP.LINE_H;
        g.globalAlpha = 0.3 + pulse * 0.3; g.strokeStyle = '#fff'; g.lineWidth = Math.max(1.5, tile * 0.055);
        for (let k = -1; k <= 1; k++) {
          g.beginPath();
          if (horiz) { g.moveTo(bx, cy + k * tile * 0.15); g.lineTo(bx + bw, cy + k * tile * 0.15); }
          else { g.moveTo(cx + k * tile * 0.15, by); g.lineTo(cx + k * tile * 0.15, by + bh); }
          g.stroke();
        }
        g.globalAlpha = 1;
      }
      g.restore();
      // centre glyph (↔ / ↕ / bomb / rainbow) with a glow so it pops
      g.shadowColor = '#fff'; g.shadowBlur = 12 + pulse * 8;
      g.fillStyle = '#fff'; g.font = 'bold ' + (tile * 0.4) + 'px system-ui';
      g.textAlign = 'center'; g.textBaseline = 'middle';
