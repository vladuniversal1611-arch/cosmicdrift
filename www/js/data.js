...
(function (global) {
  ...
  function buildLevels() {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const cols = 8, rows = 8;
      // Difficulty ramps in over the first ~250 levels (faster than before) so
      // players feel real pressure by level 20 rather than level 500.
      const d = Math.min(1, i / 250);                             // 0 → 1
      const wave = 0.5 + 0.5 * Math.sin(i * 0.7);                 // 0..1 per-level variation
      // Only the first 3 levels are 4-colour tutorials; 5 colours from lvl 4;
      // full 6-colour boards from lvl 15+.  More colours = fewer accidental
      // 3-matches = tighter play required.
      const colors = i < 3 ? 4 : (i < 15 ? 5 : 6);
      const isBoss    = inIsland === (LEVELS_PER_ISLAND - 1);
      // Sprinkled "hard" slots at 5th/10th/15th of each island so each island
      // has a rhythm of easy → normal → hard rather than a flat difficulty.
      const isHard    = !isBoss && (inIsland === 4 || inIsland === 9 || inIsland === 14);
      // Last 3 non-boss slots are the tense run-up (like Candy Crush's ★ walls).
      const isPreBoss = !isBoss && inIsland >= LEVELS_PER_ISLAND - 3;
      // MOVES: much tighter than before.  Tutorial levels get some breathing
      // room, then the budget contracts fast.  Baseline 14-18, with hard slots
      // shaving another move or two off.
      let moves;
      if (i < 3)      moves = 20;                                 // levels 1-3: teach the mechanics
      else if (i < 8) moves = 18;                                 // early wins should still feel earned
      else            moves = Math.max(13, Math.round(18 - d * 4 + wave * 2));  // 13-19
      if (isHard)    moves -= 2;
      if (isPreBoss) moves -= 1;
      ...
      if (objective === OBJ.COLLECT) {
        color = i % colors;
        // 2.5-6 gems of one colour per move (was ~0.7-4.7 before).  Forces
        // deliberate play toward the target colour instead of random matching.
        const perMove = 2.5 + d * 3.5 + wave * 0.6;               // 2.5 → 6.6
        target = Math.max(10, Math.round(moves * perMove * 0.85));
        chains = i < 6 ? 0 : Math.min(12, 1 + Math.floor(d * 9) + (isHard ? 3 : 0));
      } else if (objective === OBJ.JELLY) {
        // ~0.8-2.4 jelly cells per move — most of the board is jelly at higher
        // levels, so every move must count.
        const perMove = 0.8 + d * 1.6 + wave * 0.25;              // 0.8 → 2.65
        jellyCount = Math.max(10, Math.round(moves * perMove));
        target = jellyCount;
      } else if (objective === OBJ.ICE) {
        const perMove = 0.7 + d * 1.7 + wave * 0.25;              // 0.7 → 2.65
        ...
        chains = (inIsland % 5 === 1 && i >= 6) ? Math.min(10, 2 + Math.floor(d * 8)) : 0;
      }

      // Sprinkled crates on later non-ice, non-boss levels (deterministic
      // sin() so the pattern is stable — no Math.random() in level generation).
      if (i >= 20 && !isBoss && objective !== OBJ.ICE) {
        const p = 0.5 + 0.5 * Math.sin(i * 0.31);
        if (p > 0.72) crates = Math.min(6, 1 + Math.floor(d * 4));
      }

      if (isBoss) {
        objective = OBJ.BOSS;
        moves += 5;                                               // was +8
        // Boss HP ramps HARDER: island 0 boss ≈ 180, island 5 boss ≈ 460,
        // island 20+ ≈ 900+.  Cap keeps late bosses reasonable.
        target = Math.min(2000, Math.round(180 + island * 55 + d * 380));
        color = 0; iceCount = 0; jellyCount = 0;
        chains = Math.min(5, 1 + biome); crates = Math.max(0, Math.min(4, biome));
      }

      // Tighter star thresholds — 3-star play now REQUIRES specials/combos,
      // not just brute-force matching.  Was 1.4x / 1.9x.
