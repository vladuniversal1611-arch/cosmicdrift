...
(function (global) {
  ...
  const UI = {
    ...
    renderHome: function () {
      ...
      // Secondary-feature shortcuts moved to the map edge rails (see mapRails).
      s.appendChild(acts);
    ...
    renderMap: function () {
      ...
      for (let isl = 0; isl <= lastIsle; isl++) {
        ...
        const sign = el('div', 'isle-sign' + (isl === curIslandId ? ' cur' : ''));
        sign.style.top = (yAt(ti) - 52) + 'px';
      ...
      const buildLevel = function (i, box) {
        ...
        if (haveArt) {
          ...
          platImg = '<img class="lv-plat" src="' + MS(sprite) + '" alt="">';
        }
        node.innerHTML = platImg + '<span class="lv-num">' + (lv.boss ? '👑' : lvNum) + '</span>' +
