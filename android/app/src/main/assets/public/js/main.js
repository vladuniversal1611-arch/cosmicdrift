...
(function (global) {
  ...
  const Game = {
    ...
    boot: function () {
      ...
      // Best-effort portrait lock for WebView / installed-PWA / fullscreen
      // contexts (the native app also hard-locks portrait via the manifest and
      // MainActivity). Wrapped in try/catch — plain browsers reject it.
      try {
        if (screen.orientation && screen.orientation.lock) {
          const pl = screen.orientation.lock('portrait');
    ...
    bossDialogue: function (lv) {
      ...
      const self = this;
      setTimeout(function () { global.UI.dialogue(global.UI.spriteGlyph(lv.bossDef.sprite, lv.bossDef.emoji, '', lv.bossDef.color), D.bossName(lv.bossDef), T('boss_l_' + key)); }, 900);
    },
    ...
    startLevel: function (lvNum) {
      ...
      this.engine.init(lv, p.equipped, {
        onScore: function (s) {
          ...
          if (!self.hud || !self.hud.score) return; self.hud.score.textContent = s;
        },
    ...
    buildHud: function (lv) {
      ...
      this.hud = {
        ...
        score: top.querySelector('#hud-score'),
        moves: top.querySelector('#hud-moves'),
        movesLabel: top.querySelector('#hud-moves-label'),
    ...
    renderBossBar: function (hp, max, def) {
      ...
      panel.querySelector('.boss-name').textContent = D.bossName(def);
    ...
    pause: function () {
      const self = this;
      const svg = function (p) { return '<svg viewBox="0 0 24 24" class="picsvg"><path d="' + p + '"/></svg>'; };
      const icoPlay = svg('M8 5.2v13.6L19 12z');
      ...
      global.UI.showPause({
        buttons: [
          ...
          { label: T('to_map'), icon: icoHome, onClick: function () { self.go('map'); } }
        ]
      });
    },
    ...
    checkUnlocks: function () {
      ...
      D.ISLANDS.forEach(function (isl) {
        if (isl.unlockLevel > 0 && p.levelProgress === isl.unlockLevel + 1 && !p['_isleNotified_' + isl.id]) {
          ...
          setTimeout(function () { global.UI.toast(T('isle_unlocked', { name: D.islandName(isl) })); }, 1500);
        }
      });
    },
    ...
    resize: function () {
      ...
      if (this.engine) {
        ...
        // Board sits below the framed objective/fever panels + booster row.
        const topReserve = Math.max(304 + bossExtra + sat, h * 0.20);
    ...
    // ---- Loop -------------------------------------------------------------
    loop: function (ts) {
      // The whole frame is wrapped so a stray error can never kill the rAF
      // chain (which would freeze the game). We always reschedule at the end.
      try {
        ...
        if (global.console && console.error) console.error('loop frame error', e);
      }
      requestAnimationFrame(this._loop);
    },
