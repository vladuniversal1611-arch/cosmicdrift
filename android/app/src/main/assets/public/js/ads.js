...
(function (global) {
  ...
  // UMP (User Messaging Platform) consent — required by Google Play for
  // personalized ads in the EEA/UK. Shows the consent form once when required;
  // never blocks ads if the API/step is unavailable or fails.
  ...
  function ensureInit() {
    ...
    const AdMob = plugin();
    initing = requestConsent(AdMob)
      .then(function () { return AdMob.initialize({ initializeForTesting: CONFIG.testing }); })
